...
`npm run verify` runs 449 adversarial checks headlessly in a few seconds. Run it first, if the
...
Two attempts at this have now been removed, and the second is worth recording because it was built
and then taken out.

The first tagged every listing, statement and identity with a language and offered a picker, while
nothing translated and nothing filtered on it. The control promised a capability that did not exist:
post in Hausa, expect Hausa speakers to find it, and nothing whatsoever happens. A control that
misleads is worse than an absent one, so it went.

The second added peer-written translations: a signed record beside the original, written by someone
who reads both languages and payable in time credits like any other hour. It worked and it was
removed as out of scope. Records from either attempt may still carry a `language` or `lang` field,
and old ops may carry a `translation` entity. All of it is ignored, and all of it still verifies,
because signatures cover the bytes as they were written rather than the fields the schema declares
today.

Name the cost plainly: **a commons built to bridge divides, in a country with hundreds of languages,
speaks one of them.** A Hausa statement is invisible to a Yorùbá speaker in the same market. That is
a real limit on the mission, not a rough edge, and it is stated here rather than papered over with a
...
`npm run verify`: 449 checks across nine suites, each an attack or a field failure.
...
limits described under [Limits](#limits): it speaks English only, and there is no appeals process.
